...
import { closeTag, selectMenuList, setCurrentMenu } from "../../store/reducers/teb"
...
const CommonTag = () =>{
  ...
  const setTag=(flag,item,index)=>{
    ...
    return (
      flag?
      <Tag color="#55acee" closeIcon onClose={()=>handcloseTag(item,index)} key={item.name}>{item.label}</Tag>
      :
      <Tag key={item.name} onClick={()=>handChange(item)}>{item.label}</Tag>
    )
  }
  return (
